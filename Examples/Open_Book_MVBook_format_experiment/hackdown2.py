#!/usr/bin/env python
# Hacked up version of markdown2 that emits formatting noncharacters for Babel
...
class Markdown(object):
    ...
    def _do_italics_and_bold(self, text):
        ...
        if "code-friendly" in self.extras:
            text = self._code_friendly_strong_re.sub("\uFDD1\\1\uFDD0", text)
            text = self._code_friendly_em_re.sub("\uFDD3\\1\uFDD2", text)
        else:
            text = self._strong_re.sub("\uFDD1\\2\uFDD0", text)
            text = self._em_re.sub("\uFDD3\\2\uFDD2", text)
        return text
